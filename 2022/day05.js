...
let crates = [
  ...
  const getInput = async function () {
    ...
    function moveCrate9001(crates) {
      for (let m = 0; m < mv.length; m++) {
        let numberToSplice = Number(mv[m].amount);
        let to = Number(mv[m].to) - 1;
        let from = Number(mv[m].from) - 1;
  
        crates[to].push(
          ...crates[from].splice(crates[from].length - numberToSplice)
        );
      }
    }
  
    moveCrate9001(crates);
    getAnswer();
...
//       let indexCrateTo = Number(mv[m].to) - 1;
//       let indexCrateFrom = Number(mv[m].from) - 1;
